...
protected:
  ...
  bool target_recognition_callback(recongnition::GetTargetPose::Request& req,
    ...
    try
    {
      ...
      transform_listener_ptr->lookupTransform(target,source,ros::Time::now() - ros::Duration(0.4f),trg_to_src_stamped);
    ...
    float min = world_to_ar_tf.getOrigin().x() - 0.1f*box_length_;
    float max = world_to_ar_tf.getOrigin().x() + 0.1f*box_length_;
    ...
    min = world_to_ar_tf.getOrigin().y()- 0.1f*box_width_;
    max = world_to_ar_tf.getOrigin().y() + 0.1f*box_width_;;
