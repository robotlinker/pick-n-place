...
void AuboDriver::AuboAPICallback(const std_msgs::Float32MultiArray::ConstPtr &msg)
{
    if(controlMode == aubo_driver::SendTargetGoal)
    {
        ...
        if(type == 0)
        {
            // move to a joint target (joint motion)
        ...
        else if(type == 2)
        {
            ...
            aubo_robot_namespace::CoordCalibrateByJointAngleAndTool coordCalibrate;
            coordCalibrate.coordType = aubo_robot_namespace::BaseCoordinate;
            double rotateAngle = msg->data[1];
            double rotateAxis[3] = {msg->data[2], msg->data[3], msg->data[4]};
            if(ControllerConnectedFlag)
            {
                // ========= Add by WCY =========
                robotService.robotServiceInitGlobalMoveProfile(); // initialization of max velocity and max acceleration
                aubo_robot_namespace::JointVelcAccParam moveMaxVelc;
                aubo_robot_namespace::JointVelcAccParam moveMaxAcc;
                moveMaxVelc.jointPara[0] = 0.5; // increase max velocity of joint 4
                moveMaxAcc.jointPara[0] = 0.5; //increase max acceleration of joint 4
                moveMaxVelc.jointPara[1] = 0.7; // increase max velocity of joint 5
                moveMaxAcc.jointPara[1] = 0.7; //increase max acceleration of joint 5
                moveMaxVelc.jointPara[2] = 1.2; // increase max velocity of joint 6
                moveMaxAcc.jointPara[2] = 1.2; //increase max acceleration of joint 6
                moveMaxVelc.jointPara[3] = 1.2; // increase max velocity of joint 4
                moveMaxAcc.jointPara[3] = 1.2; //increase max acceleration of joint 4
                moveMaxVelc.jointPara[4] = 1.2; // increase max velocity of joint 5
                moveMaxAcc.jointPara[4] = 1.2; //increase max acceleration of joint 5
                moveMaxVelc.jointPara[5] = 1.2; // increase max velocity of joint 6
                moveMaxAcc.jointPara[5] = 1.2; //increase max acceleration of joint 6
                robotService.robotServiceSetGlobalMoveJointMaxVelc(moveMaxVelc); // get max velocity
                robotService.robotServiceSetGlobalMoveJointMaxAcc(moveMaxAcc); // get max acceleration
                robotService.robotServiceGetGlobalMoveJointMaxVelc(moveMaxVelc); // get max velocity
                robotService.robotServiceGetGlobalMoveJointMaxAcc(moveMaxAcc); // get max acceleration
                std::cout << moveMaxVelc.jointPara[4] << std::endl;                
                int ret = robotService.robotServiceRotateMove(coordCalibrate, rotateAxis, rotateAngle, true);
                ROS_INFO("move to the goal with API! %d", ret);
                if(ret == 0)
                {
                    ROS_INFO("move to the goal with API!");
        ...
        {
            // move to a pose target (joint motion)
            aubo_robot_namespace::CoordCalibrateByJointAngleAndTool coordCalibrate;
            coordCalibrate.coordType = aubo_robot_namespace::BaseCoordinate;
            aubo_robot_namespace::Pos coordPose;
            coordPose.x = msg->data[1];
            coordPose.y = msg->data[2];
            coordPose.z = msg->data[3];
            aubo_robot_namespace::ToolInEndDesc eefPose;
            eefPose.toolInEndPosition.x = 0;
            eefPose.toolInEndPosition.y = 0;
            eefPose.toolInEndPosition.z = 0;
...
int main(int argc, char **argv)
{
    ...
    ROS_WARN("Start the aubo_driver");
    AuboDriver aubodriver;
    aubodriver.run();
